...
<form method="POST" action="<?php echo routing::getInstance()->getUrlWeb('animal', ((isset($objAnimal)) ? 'update' : 'create' ))?>">
   ...
   <option <?php echo (isset($objOrdeno[0]->$id_animal_o) === true and $objOrdenno[0]->$id_animal_o == $animal->$id_animal) ? 'selected' : ((session::getInstance()->hasFlash(ordennoTableClass::getNameField(ordennoTableClass::ID_ANIMAL, TRUE)) === TRUE) ? '' : (request::getInstance()->hasPost(ordennoTableClass::getNameField(animalTableClass::ID_ESTADO, TRUE)) and request::getInstance()->getPost(animalTableClass::getNameField(animalBaseTableClass::ID_ESTADO, TRUE)) == $estado->$id_estado) ? 'selected' : '') ?> value="<?php echo $estado->$id_estado?>"><?php echo $estado->$descripcionestado?></option>
       <?php endforeach;?>
   </select>
    </div>
   <br>
