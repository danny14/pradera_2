...
<form method="POST" action="<?php echo routing::getInstance()->getUrlWeb('estado', ((isset($objEstado)) ? 'update' : 'create' ))?>">
    ...
    <div class="form-group <?php echo (session::getInstance()->hasFlash(estadoTableClass::getNameField(estadoTableClass::DESCRIPCION, TRUE)) === TRUE) ? 'has-error has-feedback' : ''; ?>">
        ...
        <input class="form-control" type="text" value="<?php echo ((isset($objEstado)) ? $objEstado[0]->$descripcion : ((session::getInstance()->hasFlash(estadoTableClass::getNameField(estadoTableClass::DESCRIPCION, TRUE)) === TRUE) ?  '' :  (request::getInstance()->hasPost(estadoTableClass::getNameField(estadoTableClass::DESCRIPCION, TRUE))) ? request::getInstance()->getPost(estadoTableClass::getNameField(estadoTableClass::DESCRIPCION, TRUE))  : '' ) ) ?>" name="<?php echo estadoTableClass::getNameField(estadoTableClass::DESCRIPCION, TRUE) ?>" maxlength="140">
