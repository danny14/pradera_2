...
<form method="POST" action="<?php echo routing::getInstance()->getUrlWeb('ciudad', ((isset($objCiudad)) ? 'update' : 'create' ))?>">
   ...
   <input class="form-control" type="text" value="<?php echo ((isset($objCiudad)) ? $objCiudad[0]->$descripcion : ((session::getInstance()->hasFlash(ciudadTableClass::getNameField(ciudadTableClass::DESCRIPCION, TRUE)) === TRUE) ?  '' :  (request::getInstance()->hasPost(ciudadTableClass::getNameField(ciudadTableClass::DESCRIPCION, TRUE))) ? request::getInstance()->getPost(ciudadTableClass::getNameField(ciudadTableClass::DESCRIPCION, TRUE))  : '' ) ) ?>" name="<?php echo ciudadTableClass::getNameField(ciudadTableClass::DESCRIPCION, true )?>" maxlength="140">
