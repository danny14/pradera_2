...
<form method="POST" action="<?php echo routing::getInstance()->getUrlWeb('tipo_insumo', ((isset($objTipoInsumo)) ? 'update' : 'create' ))?>">
   ...
   <input class="form-control" type="text" value="<?php echo ((isset($objTipoInsumo)) ? $objTipoInsumo[0]->$descripcion : ((session::getInstance()->hasFlash(tipoInsumoTableClass::getNameField(tipoInsumoTableClass::DESCRIPCION, TRUE)) === TRUE) ?  '' :  (request::getInstance()->hasPost(tipoInsumoTableClass::getNameField(tipoInsumoTableClass::DESCRIPCION, TRUE))) ? request::getInstance()->getPost(tipoInsumoTableClass::getNameField(tipoInsumoTableClass::DESCRIPCION, TRUE))  : '' ) ) ?>" name="<?php echo tipoInsumoTableClass::getNameField(tipoInsumoTableClass::DESCRIPCION, true )?>" maxlength="140">
